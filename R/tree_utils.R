...
phylogeny_to_newick <- function(phylogeny, mode="mu") {
    ...
    for (index in 1:(n_nodes-1)) {
        ...
        print("A")
        print(c(index, parent))
        if (is.null(merge_nodes[[parent]])) {
            print("Y")
            print(parent)
    ...
    # for (i in 1:n_nodes) {
    #     left <- i
    #     parent <- phylogeny[left, 2]  # Get the parent
    #     if (branches[parent] == "") {
    #         # Empty string
    ...
    # # Similar approach from biophybreak
    # for (i in 1:n_nodes) {
    #     branch[i] <- paste0("(",left_name, ":",left_dist_to_root, ",",
    #                         right_name,":", right_dist_to_root, ")")
    # }
    # Complete the tree
