...
#' - mutation_rate: The mutation rate in per nt per year per nucleotide.
...
#'
#' @return list with key "matrix" containing the resulting pairwise distance matrix.
#' @export
#'
#' @examples
...
simulate_classic_HIV <- function(params) { # nolint: object_name_linter
  ...
  # Use a uniform lifespan for the simulation
  uniform_lifespan_sampler <- SEEPS::uniform_lifespan_sampler(vmin = 13, vmax = 36)
  ...
  simulator_result <- SEEPS::gen_transmission_history_exponential_constant(
    ...
    lifespan_sampler = uniform_lifespan_sampler,
