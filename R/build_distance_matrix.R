...
#'
#' @param geneology A matrix with columns: parent, child, time, rate, mean, leaf, name
...
geneology_to_distance_matrix_classic <- function(geneology, spike_root = FALSE) { # nolint: cyclocomp_linter
    ...
    # spike_root option should add a row and column for a node at the root. Not yet implemented.
...
#'
#' @param geneology A matrix with columns: parent, child, time, rate, mean, leaf, name
#' @param mode Which description of distance to use. Default is "mu", for
#' mutation. Also accepts "mean", for the expected distance.
#' @param spike_root Not implemented. Default FALSE. Will exit if `TRUE`.
#'
#' @export
...
#' @seealso `SEEPS::fasta_string_to_dataframe`.
#'
#' Convert a dataframe of sequences to a pairwise distance matrix.
#' Use ape to compute a distance matrix using the specified distance model.
#' Default is the "TN93" model.
#'
#' @param df A dataframe with a column of names and a column of sequences.
#' @param model The distance model to use. Default is "TN93"
#' @param keep_root If TRUE, keep the root node in the distance matrix. Default is FALSE.
#'
