    ...
    sim.coal.tree.m <- function(tt = NULL, # nolint: object_name_linter
      ...
      if (is.null(tt)) {
            msg <- paste(
            "No way to build transmission tree: Either a transmission tree",
    ...
    if (any(c(a, b, rhoD, rhoR) < 0)) stop("Values of a, b, rhoD, and rhoR must be non-negative")
      ...
      while (k_all[1] > 1 || (rhoD_any && t[i] - tt$t_inf[2] > -1e-15) || sum(k_all[2:nInd]) > 0) {
            ...
            z[i, k_atleast2] <- Fz(
            runif(nk_atleast2), k[k_atleast2], a[k_atleast2],
             b[k_atleast2], (t[i] - tt$t_inf[k_atleast2]) * gen.rate
        ) / gen.rate
            ...
            if (rhoD_any) {
              for (j in 1:nInd) {
                    if (rhoD[j] > 0 && k[j] >= 1 &&
                      ...
                      mD[i, j] <- Fm(
                        runif(1), k[j], rhoD[j], a[j], b[j], # nolint: object_name_linter
                           (t[i] - tt$t_inf[j]) * gen.rate
                    ) / gen.rate
                    }
              }
            }
            ...
            for (j in k_nonzero) {
              if (j != 1 && t_t_inf_diffs[j] >= 0 && t[i] <= tr_window_times[j]) {
                    if (rhoR[j] > 0) {
                      mR[i, j] <- min(
                        t_t_inf_diffs[j], # nolint: object_name_linter
                          Fm(
                            runif(1), k[j], rhoR[j], a[j], b[j],
                                (t[i] - tt$t_inf[j]) * gen.rate
                        ) / gen.rate
                    )
                    } else {
    ...
    result <- list(
        "tree_newick" = tree_newick, "names_newick" = t(names_newick),
